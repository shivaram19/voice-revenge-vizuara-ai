...
class ProductionPipeline:
    ...
    def __init__(
        ...
        sarvam_tts: Any = None,   # Sarvam Bulbul v3 TTS for Indian-language sessions
    ):
        ...
        self._detected_languages: Dict[str, str] = {} # BCP-47 per session
    ...
    async def on_call_start(
        ...
        # Pre-seed language if known (outbound call to a known language speaker)
        self._detected_languages[session_id] = language or ""
        if language:
            logger.info("language_preseeded", session_id=session_id, language=language)
        ...
        # Generate greeting (use pre-seeded language if known)
        with self.latency.measure("greeting"):
            ...
            greeting_audio = await self._synthesize_to_ulaw(
                greeting_text, emotion_tone=None, language=self._detected_languages.get(session_id, "")
            )
    ...
    async def on_call_end(self, session_id: str) -> Optional[CallSession]:
        ...
        self._detected_languages.pop(session_id, None)
    ...
    def _on_transcript(self, session_id: str, event: TranscriptEvent) -> None:
        ...
        if event.is_final:
            ...
            if not self._detected_languages.get(session_id):
                ...
                if lang:
                    self._detected_languages[session_id] = lang
                    logger.info(
                        "language_detected",
                        session_id=session_id,
                        language=lang,
    ...
    async def _process_utterance(self, session_id: str, transcript: str) -> None:
        ...
        try:
            ...
            with self.latency.measure("tts"):
                ...
                session_lang = self._detected_languages.get(session_id, "")
                response_audio = await self._synthesize_to_ulaw(
                    response_text, emotion_tone, situation, language=session_lang
                )
    ...
    async def _synthesize_to_ulaw(
        ...
        For Telugu sessions (`language="te-IN"`), routes to Sarvam Bulbul v3
        instead of Deepgram Aura, using plain text (Sarvam ignores SSML).
        Sample rate is read dynamically from the WAV header so both providers
        (Deepgram 24kHz, Sarvam 22050Hz) resample correctly to 8kHz μ-law.
        """
        from src.emotion.profile import EmotionalTone
        ...
        # Route to Sarvam for Telugu; Deepgram for all other languages.
        # Sarvam ignores SSML, so pass the original plain text.
        if language == "te-IN" and self.sarvam_tts is not None:
            pcm_wav = await asyncio.to_thread(
                self.sarvam_tts.synthesize, text
            )
        else:
            pcm_wav = await asyncio.to_thread(
        ...
        # Read source sample rate from WAV header — handles both Deepgram
        # (24000 Hz) and Sarvam (22050 Hz) without a hardcoded constant.
        wav_buffer = io.BytesIO(pcm_wav)
        with wave.open(wav_buffer, "rb") as w:
            source_rate = w.getframerate()
            pcm_raw = w.readframes(w.getnframes())

        pcm_8k, _ = ratecv(pcm_raw, 2, 1, source_rate, 8000, None)
