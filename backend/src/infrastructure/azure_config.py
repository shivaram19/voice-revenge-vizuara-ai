...
class AzureConfig:
    ...
    def validate(self) -> list[str]:
        ...
        Azure OpenAI keys are optional when SARVAM_API_KEY is set —
        Sarvam LLM is used as a drop-in replacement in that case.
        """
        import os
        missing = []
        # Twilio is always required for telephony
        if not self.twilio_account_sid:
            missing.append("twilio_account_sid")
        if not self.twilio_auth_token:
            missing.append("twilio_auth_token")
        # Redis host has a safe localhost default; skip unless explicitly blank
        # Azure OpenAI only required when Sarvam is not configured
